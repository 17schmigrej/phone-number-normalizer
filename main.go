...
func main() {
	...
	must(err)
	_, err = insertPhone(db, "123 926 7230")
	must(err)
	id, err := insertPhone(db, "(185) 936 7890")
	must(err)
	_, err = insertPhone(db, "(111) 456-9190")
	must(err)
	_, err = insertPhone(db, "123-456-7894")
	must(err)
	_, err = insertPhone(db, "123-456-7890")
	must(err)
	_, err = insertPhone(db, "1234567891")
	must(err)
	_, err = insertPhone(db, "(123)572-4261")
	must(err)
...
func normalize(phone string) string {
	...
	// loop through phone number string
	for _, ch := range phone {

		// only consider number characters
		if ch >= '0' && ch <= '9' {
